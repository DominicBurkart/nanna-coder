          ...
          load-container-image = if pkgs.stdenv.isLinux then
            (pkgs.writeShellApplication {
              ...
              text = ''
                ...
                if [ -L result ]; then
                  ...
                  if file "$IMAGE_PATH" | grep -q "JSON"; then
                    ...
                    docker load < "$IMAGE_PATH" 2>/dev/null || {
                      echo "Docker load failed, trying nix2container-specific approach..."
                ...
                echo "Tagging image as: ghcr.io/$REPO_NAME/$IMAGE_NAME:$TAG"
                ...
                docker tag "$IMAGE_NAME:$TAG" "ghcr.io/$REPO_NAME/$IMAGE_NAME:$TAG" 2>/dev/null || {
                  echo "Direct tag failed, trying to find loaded image..."
