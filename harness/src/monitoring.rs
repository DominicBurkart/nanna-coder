...
mod tests {
    ...
    async fn test_record_model_inference() {
        ...
        collector.record_model_inference("test-model", model_metrics).await;
        let metrics = collector.get_current_metrics().await.unwrap();
    ...
    async fn test_reset_metrics() {
        ...
        collector.record_request_latency("svc", Duration::from_millis(100)).await;
    ...
    #[tokio::test]
    async fn test_export_metrics_custom_format() {
        let collector = DefaultMetricsCollector::new();
    ...
    async fn test_get_alert_history() {
        let manager = DefaultAlertManager::new();
        manager.send_alert("Alert 1", "desc 1", AlertSeverity::Info).await.unwrap();
        manager.send_alert("Alert 2", "desc 2", AlertSeverity::Error).await.unwrap();
        let history = manager.get_alert_history(10).await.unwrap();
        assert_eq!(history.len(), 2);
        assert_eq!(history[0].title, "Alert 2");
    }
    ...
    async fn test_check_model_health() {
        ...
        assert_eq!(result.component, "model:test-model");
    }
