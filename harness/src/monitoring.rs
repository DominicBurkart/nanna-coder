...
mod tests {
    ...
    #[test]
    fn health_status_requires_attention_for_bad_states() {
    ...
    // -----------------------------------------------------------------------
    // AlertSeverity and ErrorSeverity ordering
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // record_error and record_model_inference
    // -----------------------------------------------------------------------
    ...
    async fn record_model_inference_appears_in_metrics() {
        ...
        let metrics = collector.get_current_metrics().await.unwrap();
        let recorded = metrics.model_metrics.get("test-model").unwrap();
        assert_eq!(recorded.inference_count, 10);
        assert!((recorded.tokens_per_second - 42.0).abs() < f64::EPSILON);
    }
    ...
    #[tokio::test]
    async fn export_metrics_custom_format_errors() {
        ...
        let msg = format!("{}", err);
        assert!(
            msg.contains("myformat"),
            "error message should name the format"
        );
    }

    // -----------------------------------------------------------------------
    // AlertManager: get_alert_history, configure_thresholds, error paths
    // -----------------------------------------------------------------------

    #[tokio::test]
    async fn get_alert_history_respects_limit() {
        ...
        // History with limit 3 should return at most 3 entries
        let history = manager.get_alert_history(3).await.unwrap();
        assert_eq!(history.len(), 3);

        // History with limit 10 should return all 5
        let all_history = manager.get_alert_history(10).await.unwrap();
        assert_eq!(all_history.len(), 5);
    }

    #[tokio::test]
    async fn acknowledge_alert_not_found_returns_error() {
        let manager = DefaultAlertManager::new();
    ...
    #[tokio::test]
    async fn configure_thresholds_updates_thresholds() {
        ...
        let result = manager.configure_thresholds(new_thresholds).await;
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn all_alert_severities_create_alerts() {
        let manager = DefaultAlertManager::new();
        for severity in [
            AlertSeverity::Info,
    ...
    // -----------------------------------------------------------------------
    // DefaultHealthMonitor: model health and set_check_interval
    // -----------------------------------------------------------------------

    #[tokio::test]
    async fn check_model_health_returns_healthy() {
    ...
    #[tokio::test]
    async fn set_check_interval_updates_interval() {
        let mut monitor = DefaultHealthMonitor::new(Duration::from_secs(30));
        // Just verify it doesn't panic — the field is private so we test side-effects
        monitor.set_check_interval(Duration::from_secs(60));
    }

    #[tokio::test]
    async fn comprehensive_health_check_returns_results() {
        let monitor = DefaultHealthMonitor::new(Duration::from_secs(30));
        let results = monitor.comprehensive_health_check().await.unwrap();
        // At minimum we get the system health check result
