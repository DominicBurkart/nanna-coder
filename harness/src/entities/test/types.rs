...
mod new_types_tests {
    ...
    /// `NextestUnavailable` carries no data but must still Display.
    #[test]
    fn test_test_error_display_nextest_unavailable() {
        let err = TestError::NextestUnavailable;
        let s = format!("{}", err);
        assert!(s.contains("cargo-nextest"), "got: {s}");
        assert!(s.contains("install"), "got: {s}");
    }

    /// Exercise the `metadata_mut` and `to_json` methods of `TestEntity` —
    /// the placeholder type kept around for backwards compatibility.
    #[test]
    fn test_test_entity_metadata_mut_and_to_json() {
        let mut e = TestEntity::new();
        let before = e.metadata().id.clone();
        e.metadata_mut().version += 1;
        assert_eq!(e.metadata().id, before);
        assert_eq!(e.metadata().version, 2);
        let json = e.to_json().expect("to_json");
        assert!(json.contains("\"entity_type\":\"Test\""));
    }

    /// Exercise `TestRunEntity::metadata_mut`.
    #[test]
    fn test_test_run_entity_metadata_mut() {
        let mut run = TestRunEntity::new("nextest".into());
        let before = run.metadata().id.clone();
        run.metadata_mut().tags.push("smoke".into());
        assert_eq!(run.metadata().id, before);
        assert_eq!(run.metadata().tags, vec!["smoke".to_string()]);
    }

    /// Exercise `LintResultEntity::metadata_mut` and `Default`.
    #[test]
    fn test_lint_result_entity_metadata_mut_and_default() {
        let mut lint = LintResultEntity::default();
        assert_eq!(lint.entity_type(), EntityType::Lint);
        assert!(lint.results.is_empty());
        let before = lint.metadata().id.clone();
        lint.metadata_mut().tags.push("clippy".into());
        assert_eq!(lint.metadata().id, before);
        assert_eq!(lint.metadata().tags, vec!["clippy".to_string()]);
    }

    /// `TestRunEntity::passed`/`failed` over an empty result set both return 0,
    /// exercising the iterator pipeline without invoking the matcher arms.
    #[test]
    fn test_test_run_entity_counters_on_empty() {
        let run = TestRunEntity::new("cargo".into());
        assert_eq!(run.passed(), 0);
        assert_eq!(run.failed(), 0);
    }

    /// Round-trip a `TestStatus::Failed` variant to confirm the matcher in
    /// `failed()` recognises it.
    #[test]
    fn test_test_run_entity_failed_counts_timeout_and_failed() {
        let mut run = TestRunEntity::new("nextest".into());
        run.results.push(TestResult {
            name: "a".into(),
