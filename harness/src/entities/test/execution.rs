...
mod tests {
    ...
    /// `try_read_head_sha` swallows errors and returns `None` when the path
    /// is not a git repo. Covers the function which was otherwise unreached
    /// because the happy-path of `CargoTestExecutor::run_tests` requires
    /// `cargo nextest` to be installed.
    #[test]
    fn test_try_read_head_sha_returns_none_for_non_repo() {
        let tmp = tempfile::tempdir().expect("tmpdir");
        assert!(try_read_head_sha(tmp.path()).is_none());
    }

    /// In a real git repository `try_read_head_sha` returns the HEAD sha.
    /// Build a one-commit repo in a tempdir to keep the test hermetic.
    #[test]
    fn test_try_read_head_sha_returns_some_for_repo() {
        let tmp = tempfile::tempdir().expect("tmpdir");
        let repo = git2::Repository::init(tmp.path()).expect("git init");

        // Stage an empty tree and commit it. `tree_id` of the empty index
        // is the canonical empty tree object.
        ...
        let sha = try_read_head_sha(tmp.path()).expect("should read HEAD");
        assert_eq!(sha, oid.to_string());
    }

    /// `ensure_nextest_available` invokes `cargo nextest --version`. In CI
    /// (and the project's Nix dev-shell) nextest is provisioned, so the
    /// probe returns `Ok(())`. When the probe instead fails, the function
    /// must return `NextestUnavailable` — both branches are accepted here
    /// so the test is robust across local dev environments.
    #[tokio::test]
    async fn test_ensure_nextest_available_invokes_probe() {
        let result = ensure_nextest_available().await;
        let ok = matches!(result, Ok(()) | Err(TestError::NextestUnavailable));
        assert!(ok, "unexpected probe result: {result:?}");
    }

    /// Force the failure branch of `ensure_nextest_available` by stripping
    /// `cargo` (and therefore `cargo nextest`) from `PATH` for the duration
    /// of the call. This covers the `_ => Err(NextestUnavailable)` arm
    /// regardless of whether nextest is installed on the host.
    ///
    /// `#[serial]` ensures this test does not race with other tests that
    /// rely on `PATH` (e.g. `run_cargo_success_branch`).
    #[tokio::test]
    #[serial_test::serial(env_path)]
    async fn test_ensure_nextest_available_returns_unavailable_when_cargo_missing() {
        let original = std::env::var_os("PATH");
        // SAFETY: serialised via `#[serial]` so no other test touches PATH.
        unsafe {
            std::env::set_var("PATH", "/nonexistent-dir-for-ensure-nextest-unavail");
        }
        let result = ensure_nextest_available().await;
        // Restore PATH even if assertion fails.
        unsafe {
            match original {
        ...
        assert!(
            matches!(result, Err(TestError::NextestUnavailable)),
            "expected NextestUnavailable, got {result:?}"
        );
    }

    /// `run_cargo` returns `Ok(stdout)` on success — invoke `cargo --version`
    /// which exits zero in any environment where this crate builds. This
    /// covers the success branch of `run_cargo` (otherwise unreachable
    /// without a real Rust project and `cargo nextest` installed).
    #[tokio::test]
    async fn test_cargo_test_executor_run_cargo_success_branch() {
        let tmp = tempfile::tempdir().expect("tmpdir");
        let exec = CargoTestExecutor::new(tmp.path().to_path_buf());
        let out = exec
            .run_cargo(&["--version"])
            .await
            .expect("cargo --version must succeed");
        let s = String::from_utf8_lossy(&out);
        assert!(s.contains("cargo"), "unexpected output: {s}");
    }

    /// `CargoTestExecutor::run_lints` returns `CommandFailed` when cargo
    /// runs but exits non-zero (here: no Cargo manifest in CWD). Exercises
    /// the `CommandFailed` branch of `run_cargo`.
    #[tokio::test]
    async fn test_cargo_test_executor_run_lints_command_failed_on_empty_dir() {
        let tmp = tempfile::tempdir().expect("tmpdir");
        let exec = CargoTestExecutor::new(tmp.path().to_path_buf());
        let err = exec.run_lints().await.expect_err("should fail");
        match err {
            TestError::CommandFailed { code, stderr } => {
                assert!(code != Some(0), "expected non-zero exit code, got {code:?}");
                assert!(!stderr.is_empty(), "expected stderr output");
            }
