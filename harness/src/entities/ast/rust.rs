...
mod tests {
    ...
    #[test]
    fn test_parse_rust_source_empty_file() {
        // Empty source is valid Rust and must yield an entirely empty summary
        // (no items, no uses) so the `for item in &file.items` loop is
        // exercised on a zero-element collection.
        let summary = parse_rust_source("").expect("parse empty");
        assert!(summary.functions.is_empty());
        assert!(summary.structs.is_empty());
        assert!(summary.impls.is_empty());
        assert!(summary.uses.is_empty());
    }

    #[test]
    fn test_parse_rust_file_location_first_line_is_one() {
        // 1-indexed line numbers: an `fn` declared on the very first
        // line of the source must report line == 1, not 0.
        let f = write_rs("fn first() {}\n");
        let entity = parse_rust_file(f.path()).expect("parse");
        assert_eq!(entity.summary.functions[0].line, 1);
    }

    #[test]
    fn test_parse_rust_source_other_item_kinds_are_ignored() {
        // Items that are not Fn/Struct/Impl/Use (e.g. const, enum, mod,
        // trait) fall through the `_ => {}` arm and must leave the
        // summary empty.
    ...
    #[test]
    fn test_parse_rust_file_unsupported_language_message() {
        // Exercise the `Some(other)` arm with a non-Rust extension and
        // confirm the error message carries the offending extension.
        let mut f = tempfile::Builder::new()
            .suffix(".txt")
            .tempfile()
            .expect("tmp .txt");
        f.write_all(b"not rust").unwrap();
        f.flush().unwrap();
        match parse_rust_file(f.path()).expect_err("should fail") {
            AstError::UnsupportedLanguage(ext) => assert_eq!(ext, "txt"),
            other => panic!("expected UnsupportedLanguage, got {other:?}"),
        }
    }
}
