...
impl<'a> AstQuery<'a> {
    ...
    pub fn find_functions(&self, needle: &str) -> Vec<&'a FunctionSig> {
        let items = &self.entity.summary.functions;
        items.iter().filter(|f| f.name.contains(needle)).collect()
    }

    /// Find all structs whose name contains `needle` as a substring.
    pub fn find_structs(&self, needle: &str) -> Vec<&'a StructSig> {
        let items = &self.entity.summary.structs;
        items.iter().filter(|s| s.name.contains(needle)).collect()
    }
    ...
    pub fn find_impls(&self, needle: &str) -> Vec<&'a ImplSig> {
        let items = &self.entity.summary.impls;
        items
            .iter()
            .filter(|i| {
...
mod tests {
    ...
    #[test]
    fn test_find_on_empty_summary() {
        // Empty AstSummary: every finder must return an empty Vec without
        // panicking and without short-circuiting on the trait_name branch.
        let entity = RustAstEntity::new(PathBuf::from("empty.rs"), AstSummary::default(), "");
        let q = AstQuery::new(&entity);
        assert!(q.find_functions("").is_empty());
        assert!(q.find_functions("anything").is_empty());
        assert!(q.find_structs("").is_empty());
        assert!(q.find_impls("").is_empty());
    }

    #[test]
    fn test_find_functions_empty_needle_matches_all() {
        // An empty needle is a substring of every string, so every item
        // in the summary should be returned. Exercises the `true` branch
        // of every filter closure uniformly.
        let entity = fixture();
        let q = AstQuery::new(&entity);
        assert_eq!(q.find_functions("").len(), 2);
        assert_eq!(q.find_structs("").len(), 2);
        assert_eq!(q.find_impls("").len(), 2);
    }
}
