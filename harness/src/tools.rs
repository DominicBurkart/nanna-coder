...
mod tests {
    ...
    #[test]
    fn test_canonicalize_workspace_root_valid() {
        let temp_dir = std::env::temp_dir().join("nanna_test_canon_valid");
        std::fs::create_dir_all(&temp_dir).unwrap();
        let result = canonicalize_workspace_root(&temp_dir);
        assert!(result.is_ok());
        std::fs::remove_dir_all(&temp_dir).unwrap();
    }

    #[test]
    fn test_canonicalize_workspace_root_invalid() {
        let nonexistent = PathBuf::from("/tmp/nanna_nonexistent_path_that_does_not_exist");
        let result = canonicalize_workspace_root(&nonexistent);
        assert!(result.is_err());
        match result {
            Err(ToolError::PathSecurityViolation { .. }) => {}
            _ => panic!("Expected PathSecurityViolation error"),
    ...
    fn test_resolve_path_relative() {
        ...
        let resolved = resolve_path(relative, workspace);
    ...
    fn test_resolve_path_absolute() {
        let workspace = Path::new("/workspace");
        let absolute = Path::new("/other/file.rs");
        let resolved = resolve_path(absolute, workspace);
        assert_eq!(resolved, absolute.to_path_buf());
    }

    #[test]
    fn test_validate_path_for_write_rejects_dotdot() {
        let temp_dir = std::env::temp_dir().join("nanna_test_write_dotdot");
        std::fs::create_dir_all(&temp_dir).unwrap();
        let result = validate_path_for_write(Path::new("../escape"), &temp_dir);
        assert!(result.is_err());
        match result {
            Err(ToolError::PathSecurityViolation { .. }) => {}
            _ => panic!("Expected PathSecurityViolation error"),
    ...
    #[test]
    fn test_validate_path_for_write_rejects_symlink_outside() {
        let temp_dir = std::env::temp_dir().join("nanna_test_write_symlink");
        std::fs::create_dir_all(&temp_dir).unwrap();
        let symlink_path = temp_dir.join("escape_link");
        let _ = std::fs::remove_file(&symlink_path);
        std::os::unix::fs::symlink("/etc", &symlink_path).unwrap();
        let result = validate_path_for_write(Path::new("escape_link/passwd"), &temp_dir);
        assert!(result.is_err());
        match result {
            Err(ToolError::PathSecurityViolation { .. }) => {}
            _ => panic!("Expected PathSecurityViolation error"),
    ...
    #[test]
    fn test_validate_path_within_workspace_rejects_symlink_outside() {
        let temp_dir = std::env::temp_dir().join("nanna_test_read_symlink");
        std::fs::create_dir_all(&temp_dir).unwrap();
        let symlink_path = temp_dir.join("escape_link");
        let _ = std::fs::remove_file(&symlink_path);
        std::os::unix::fs::symlink("/etc/hostname", &symlink_path).unwrap();
        let result = validate_path_within_workspace(Path::new("escape_link"), &temp_dir);
        assert!(result.is_err());
        match result {
            Err(ToolError::PathSecurityViolation { .. }) => {}
            _ => panic!("Expected PathSecurityViolation error"),
