...
`harness::action_auditor::RuleActionAuditor` consults a `WindowSet` directly for every `Sandbox`/`Production` action review — but *which* set matters: `TaskManager`'s own default action gate (`default_action_gate` in `harness/src/task.rs`) constructs its `RuleActionAuditor` with `WindowSet::default()`, an empty set, and nothing in `harness/src/main.rs` calls `TaskManager::with_action_gate` (the builder that would replace it) with a `WindowSet` loaded from `windows.toml`. So as wired today, a live agent task's `Sandbox`/`Production` actions run against an empty window set unless a caller supplies its own action gate — every call site that does pass a real `WindowSet` into a `RuleActionAuditor`/`ActionGate` in this codebase is a test. The CLI's own `deploy` subcommand does load `windows.toml` (see [Rollout State Machine](#rollout-state-machine) below), but that is a separate `WindowSet` instance consulted by the rollout executor, not by the action gate.
