...
- **Never merge a pull request or close an issue.** This mirrors the standing human-operator policy for this project (plan or review approval is not merge or close authorization) applied to autonomous agents specifically. No tool in the catalog can merge a pull request or close an issue at all: `harness::pr_tools` exposes `github_pr_open` (always opens a draft), `github_pr_promote` (marks an identity's own draft ready for review — not a merge), `github_pr_close` (closes a pull request the identity itself owns, e.g. to abandon its own superseded draft, and requires the closing comment to reference the originating issue), `github_issue_read` and `github_issue_comment`. There is no `github_pr_merge` and no tool that closes an issue. A human merges pull requests and closes issues.
