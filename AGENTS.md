...
- **Never merge a pull request or close an issue.** This mirrors the standing human-operator policy for this project (plan or review approval is not merge or close authorization) applied to autonomous agents specifically. No tool in the catalog can merge a pull request, close an issue, or enable auto-merge: `harness::pr_tools` exposes `github_pr_open` (always opens a draft), `github_pr_promote` (marks an identity's own draft ready for review — not a merge), `github_pr_close` (closes a pull request the identity itself owns, e.g. to abandon its own superseded draft, and requires the closing comment to reference the originating issue), `github_pr_comments` (a read-only status/review report that includes whether GitHub's auto-merge is enabled, but has no tool to change it), `github_issue_read` and `github_issue_comment`. There is no `github_pr_merge`, no tool that closes an issue, and no tool that toggles auto-merge. The most direct way an agent could otherwise merge by other means — pushing straight to the default branch — is separately refused: `git_push_branch` reads the repository's actual default branch and refuses to push to it. A human merges pull requests and closes issues.
